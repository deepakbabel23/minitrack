...
- [.claude/agents/code-reviewer.md](.claude/agents/code-reviewer.md) is a read-only reviewer used in the *Review* step. It checks correctness vs. the goal, missing 404s/edge cases, validation gaps, and whether a test exists and `pytest -q` passes — and deliberately ignores style nits.
- [.claude/agents/scaffold-router.md](.claude/agents/scaffold-router.md) scaffolds a brand-new REST resource (schema, exception, repository, service, router, tests, and a spec.md contract section) across every layer in dependency order, mirroring the existing `tasks` resource. Use it for the *Implement* step when adding a new resource.
